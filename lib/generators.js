 ...
 * @param {function} cb
 */
function generateAction(path, modelName, generateMethod, cb) {
    var action = ft.loadTemplateSync('action.js');
    action = action.replace(/{name}/g, modelName);
    action = action.replace(/{urlName}/g, modelName.toLowerCase());
    action = action.replace(/{actionType}/g, formatTools.uppercase(modelName));
    action = action.replace(/{actionName}/g, modelName.toLowerCase() + 'Action');

    if (generateMethod === 't') {
        ft.createDirIfIsNotDefined(path, modelName, function () {
            ft.writeFile(path + '/app/' + modelName + '/' + modelName.toLowerCase() + 'Action.js', action, null, cb);
        });
    } else {
        ft.createDirIfIsNotDefined(path, modelName, function () {
            ft.writeFile(path + '/app/' + modelName + '/' + modelName.toLowerCase() + 'Action.js', action, null, cb);
 ...
 * @param {function} cb
 */
function generateComponent(path, modelName, generateMethod, cb) {
    var component = ft.loadTemplateSync('component.js');
    component = component.replace(/{name}/g, modelName);
    component = component.replace(/{componentName}/g, modelName.toLowerCase() + 'Action');

    if (generateMethod === 't') {
        ft.createDirIfIsNotDefined(path, modelName, function () {
            ft.writeFile(path + '/app/' + modelName + "/Component/" + modelName + '.js', component, null, cb);
        });
    } else {
        ft.createDirIfIsNotDefined(path, modelName, function () {
            ft.writeFile(path + '/app/' + modelName + "/Component/" + modelName + '.js', component, null, cb);
 ...
 * @param {function} cb
 */
function generateCSS(path, modelName, generateMethod, cb) {
    var css = ft.loadTemplateSync('css.js');
    css = css.replace(/{name}/g, modelName);

    if (generateMethod === 't') {
        ft.createDirIfIsNotDefined(path, modelName, function () {
            ft.writeFile(path + '/app/' + modelName + "/Component/" + modelName + '.css', css, null, cb);
        });
    } else {
        ft.createDirIfIsNotDefined(path, modelName, function () {
            ft.writeFile(path + '/app/' + modelName + "/Component/" + modelName + '.css', css, null, cb);
